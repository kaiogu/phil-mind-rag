...
def _result(text: str, source: str = "nagel.pdf", section: str = "Intro") -> RetrievalResult:
    return RetrievalResult(text=text, score=0.9, metadata={"source": source, "section": section})
...
class TestRAGPrompt:
    ...
    def test_prompt_contains_source_citation(self) -> None:
        built = RAGPrompt().build("Q?", [_result("text", source="nagel.pdf", section="Section 2")])
        assert "nagel.pdf" in built
        assert "Section 2" in built

    def test_prompt_includes_all_contexts(self) -> None:
        contexts = [_result("First passage."), _result("Second passage.", section="Conclusion")]
